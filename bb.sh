...
function savechanges {
    sed -i "s/^sbunnynmask.*/sbunnynmask=$sbunnynmask/" $0
    sed -i "s&^sbunnynet.*&sbunnynet=$sbunnynet&" $0
    sed -i "s/^sbunnylan.*/sbunnylan=$sbunnylan/" $0
    sed -i "s/^sbunnywan.*/sbunnywan=$sbunnywan/" $0
    sed -i "s/^sbunnygw.*/sbunnygw=$sbunnygw/" $0
    sed -i "s/^sbunnyhostip.*/sbunnyhostip=$sbunnyhostip/" $0
    sed -i "s/^sbunnyip.*/sbunnyip=$sbunnyip/" $0
...
function connectsaved {
    ...
    iptables -F
    iptables -A FORWARD -i $sbunnywan -o $sbunnylan -s $sbunnynet -m state --state NEW -j ACCEPT #setup IP forwarding
